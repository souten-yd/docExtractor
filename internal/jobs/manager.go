...
func (m *Manager) run(it item,workerID int){
	m.mu.Lock();j:=m.jobs[it.id];if j==nil||j.State==Cancelled{m.mu.Unlock();return};ctx,cancel:=context.WithCancel(m.rootCtx);m.cancel[it.id]=cancel;j.State=Running;j.Worker=workerID;j.StartedAt=time.Now().UTC();m.mu.Unlock()
	tx,prepErr:=beginOverwrite(it.task)
	var err error
	if prepErr!=nil{err=prepErr}else{err=m.processor(ctx,it.id,it.task,func(u Update){m.mu.Lock();if j:=m.jobs[it.id];j!=nil&&j.State==Running{if u.Stage!=""{j.Stage=u.Stage};if u.Progress>=0&&u.Progress<=1{j.Progress=u.Progress};if u.BytesRead>=0{j.BytesRead=u.BytesRead};if u.BytesWritten>=0{j.BytesWritten=u.BytesWritten};if u.Operation!=""{j.Operation=u.Operation};if u.Entries>0{j.Entries=u.Entries}};m.mu.Unlock()})}
	if tx!=nil{if finishErr:=tx.finish(err==nil);finishErr!=nil{if err==nil{err=finishErr}else{err=fmt.Errorf("%v; %w",err,finishErr)}}}
	cancel();m.mu.Lock();defer m.mu.Unlock();delete(m.cancel,it.id);j=m.jobs[it.id];if j==nil{return};j.FinishedAt=time.Now().UTC();if errors.Is(err,context.Canceled){j.State=Cancelled;j.Error="cancelled"}else if err!=nil{j.State=Failed;j.Error=err.Error()}else{j.State=Succeeded;j.Progress=1;j.Stage="done"}
}

...
type Summary struct {
	...
	ActiveIOMiBPerSec   float64 `json:"active_io_mib_per_sec"`
	CompletedIOMiBPerSec float64 `json:"completed_io_mib_per_sec"`
...
func (m *Manager) List() []Job {
	...
	for _, j := range m.jobs {
		out = append(out, decorateJob(j, now))
	}
	m.mu.RUnlock()
...
func (m *Manager) Summary() Summary {
	now := time.Now().UTC()
	s := Summary{Workers: m.workers}
	var measuredBytes int64
	var measuredDurationMS int64
	m.mu.RLock()
	for _, raw := range m.jobs {
		j := decorateJob(raw, now)
		s.Total++
		s.BytesRead += j.BytesRead
		s.BytesWritten += j.BytesWritten
		switch j.State {
		case Queued:
			s.Queued++
		case Running:
			s.Running++
			s.ActiveIOMiBPerSec += j.IOMiBPerSec
		case Succeeded:
			s.Succeeded++
			// rename-zip intentionally avoids rewriting the archive and does not
			// represent a meaningful streaming throughput measurement.
			if j.Operation != "rename-zip" && j.DurationMS > 0 && j.BytesRead+j.BytesWritten > 0 {
				measuredBytes += j.BytesRead + j.BytesWritten
				measuredDurationMS += j.DurationMS
				s.MeasuredJobs++
			}
		case Failed:
			s.Failed++
	...
	if measuredDurationMS > 0 {
		s.CompletedIOMiBPerSec = bytesPerSecondMiB(measuredBytes, measuredDurationMS)
	}
	return s
}
...
func (m *Manager) run(it item, workerID int) {
	m.mu.Lock()
	j := m.jobs[it.id]
...
func decorateJob(j *Job, now time.Time) Job {
	cp := *j
	if !cp.StartedAt.IsZero() {
		cp.QueueWaitMS = cp.StartedAt.Sub(cp.CreatedAt).Milliseconds()
		end := now
		if !cp.FinishedAt.IsZero() {
			end = cp.FinishedAt
		}
		if end.Before(cp.StartedAt) {
			end = cp.StartedAt
		}
		cp.DurationMS = end.Sub(cp.StartedAt).Milliseconds()
		if cp.Operation != "rename-zip" && cp.DurationMS > 0 {
			cp.ReadMiBPerSec = bytesPerSecondMiB(cp.BytesRead, cp.DurationMS)
			cp.WriteMiBPerSec = bytesPerSecondMiB(cp.BytesWritten, cp.DurationMS)
			cp.IOMiBPerSec = bytesPerSecondMiB(cp.BytesRead+cp.BytesWritten, cp.DurationMS)
		}
	}
	return cp
}

func bytesPerSecondMiB(bytes, durationMS int64) float64 {
	if bytes <= 0 || durationMS <= 0 {
		return 0
	}
	return (float64(bytes) / (1024 * 1024)) / (float64(durationMS) / 1000)
}

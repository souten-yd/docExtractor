...
func (p *Processor) processZIP(ctx context.Context, src, dst string, _ bool, report func(Progress)) (Result, error) {
	...
	if info.NestedArchives == 0 {
		if len(groups) == 1 && groups[0].Name != "" {
			target := outputForFolder(dst, groups[0].Name)
			res, err := moveOrCopyVerifiedZIP(ctx, src, target, false, report)
		...
		if len(groups) > 1 {
			res, err := p.splitNormalizedZIP(ctx, src, dst, "", false, report)
			if err != nil { return Result{}, err }
		...
		res, err := moveOrCopyVerifiedZIP(ctx, src, dst, false, report)
		if err == nil { report(Progress{Stage:"done", Progress:1, BytesRead:res.BytesRead, BytesWritten:res.BytesWritten}) }
		return res, err
	}

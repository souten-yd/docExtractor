...
func clusterPlans(plans []Plan, persisted map[string]string) []Plan {
	...
	for i := range plans {
		if plans[i].Series == "" || plans[i].Error != "" {
			continue
		}
	...
	for i := 0; i < len(plans); i++ {
		if plans[i].Series == "" || plans[i].Error != "" { continue }
		for j := i+1; j < len(plans); j++ {
			if plans[j].Series == "" || plans[j].Error != "" { continue }
			score, reason := sameSeries(plans[i].Series, plans[j].Series)
			if score >= 0.92 {
				union(i,j)
				if plans[i].Cluster.Reason == "" { plans[i].Cluster.Reason = reason; plans[i].Cluster.Score = score }
				if plans[j].Cluster.Reason == "" { plans[j].Cluster.Reason = reason; plans[j].Cluster.Score = score }
			}
		}
	}
	groups := map[int][]int{}
	for i := range plans { if plans[i].Series!="" && plans[i].Error=="" { r:=find(i); groups[r]=append(groups[r],i) } }
	for _, idxs := range groups {
		...
		for _, idx := range idxs {
			if _,ok:=seen[plans[idx].Series]; !ok { seen[plans[idx].Series]=struct{}{}; aliases=append(aliases,plans[idx].Series) }
		}
		...
		for _, idx := range idxs {
			...
			if plans[idx].Cluster.Reason == "" { plans[idx].Cluster.Reason = "series cluster"; plans[idx].Cluster.Score = 0.95 }
		}
	}
	return plans
}
...
func aliasLookup(aliases map[string]string, s string) (string,bool) {
	key := canonicalKey(s)
	for a,c := range aliases { if canonicalKey(a)==key { return c,true } }
	return "",false
}

func chooseCanonical(plans []Plan, idxs []int, persisted map[string]string) string {
	for _,idx:=range idxs { if c,ok:=aliasLookup(persisted,plans[idx].Series);ok{return c} }
	best:=plans[idxs[0]].Series
	for _,idx:=range idxs[1:] { cand:=plans[idx].Series; if richerSeries(cand,best){best=cand} }
	return best
}

func richerSeries(a,b string) bool {
	aj,bj:=containsJapaneseText(a),containsJapaneseText(b)
	al,bl:=containsLatin(a),containsLatin(b)
	if aj&&al && !(bj&&bl) { return true }
	if bj&&bl && !(aj&&al) { return false }
...
func sameSeries(a,b string)(float64,string){
	...
	if bilingualEquivalent(a,b) { return 0.98,"latin title plus Japanese alias" }
	if hasSpinOffMarker(a)||hasSpinOffMarker(b) { return 0,"" }
	s:=levenshteinSimilarity([]rune(ka),[]rune(kb))
	if len([]rune(ka))>=8 && len([]rune(kb))>=8 && s>=0.94 { return s,"minor filename variation" }
	return 0,""
}

func bilingualEquivalent(a,b string) bool {
	if hasSpinOffMarker(a)||hasSpinOffMarker(b){return false}
	short,long:=a,b
	if len([]rune(short))>len([]rune(long)){short,long=long,short}
	sk:=canonicalKey(short)
	if len([]rune(sk))<5 || !containsLatin(short) { return false }
	lk:=canonicalKey(long)
	if !strings.HasPrefix(lk,sk){return false}
	rest:=strings.TrimPrefix(lk,sk)
	return rest!="" && containsJapaneseText(rest)
}
...
func canonicalKey(s string) string {
	...
	s=strings.Map(func(r rune) rune {
		...
		if r>='Ａ'&&r<='Ｚ'{return r-'Ａ'+'a'}
		if r>='ａ'&&r<='ｚ'{return r-'ａ'+'a'}
		if r>='０'&&r<='９'{return r-'０'+'0'}
		return r
	},s)
	return s
}
...
func hasSpinOffMarker(s string) bool {
	l:=strings.ToLower(s)
	for _,m:=range []string{"外伝","番外","スピンオフ","spin-off","spinoff","side story","another story"}{if strings.Contains(l,m){return true}}
	return false
}
...
func levenshteinSimilarity(a,b []rune)float64{m:=len(a);n:=len(b);if m==0||n==0{return 0};prev:=make([]int,n+1);cur:=make([]int,n+1);for j:=0;j<=n;j++{prev[j]=j};for i:=1;i<=m;i++{cur[0]=i;for j:=1;j<=n;j++{cost:=0;if a[i-1]!=b[j-1]{cost=1};x:=prev[j]+1;y:=cur[j-1]+1;z:=prev[j-1]+cost;if y<x{x=y};if z<x{x=z};cur[j]=x};prev,cur=cur,prev};max:=m;if n>max{max=n};return 1-float64(prev[n])/float64(max)}

func replaceSeriesDir(destination,oldSeries,newSeries string)string{
	if destination==""||oldSeries==newSeries{return destination}
	// Destination is always <root>/<series>/<file>; replace only the parent dir name.

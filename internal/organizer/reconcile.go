...
type ReconcileItem struct {
	Source string `json:"source"`; Relative string `json:"relative"`; Series string `json:"series"`; Destination string `json:"destination"`; Action string `json:"action"`; Confidence float64 `json:"confidence"`; Reason string `json:"reason,omitempty"`; DuplicateOf string `json:"duplicate_of,omitempty"`; Size int64 `json:"size"`
}
...
func (o *Organizer) ReconcileScan(root string) (ReconcileReport,error) {
	root,err:=normalizeRoot(root);if err!=nil{return ReconcileReport{},err};raws:=make([]reconcileRaw,0)
	err=filepath.WalkDir(root,func(path string,d os.DirEntry,walkErr error)error{
		...
		parent:=filepath.Base(filepath.Dir(path));if filepath.Dir(path)!=root&&parent!="."&&parent!=""{if score,_:=sameSeries(series,parent);score>=.90{if richerSeries(parent,series){series=parent};if confidence<.90{confidence=.90}}else if confidence<o.confidenceThreshold{series=parent;confidence=.78}}
	...
	plans:=make([]Plan,len(raws));for i,r:=range raws{if r.err!=nil{plans[i]=Plan{Name:r.rel,Error:r.err.Error()}}else{plans[i]=Plan{Name:r.rel,Series:r.series,Confidence:r.confidence}}}

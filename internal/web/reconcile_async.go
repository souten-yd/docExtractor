...
func (s *Server) validateAsyncRequest(req *asyncReconcileRequest) error {
	...
	if len(req.Roots) == 0 && strings.TrimSpace(req.Root) != "" {
		req.Roots = []string{req.Root}
	}
	if len(req.Roots) == 0 || len(req.Roots) > 32 {
		return fmt.Errorf("select 1 to 32 library roots")
	}
	...
	for _, raw := range req.Roots {
		...
		if root == "." || !filepath.IsAbs(root) || !withinPath(s.browseBase(), root) {
			return fmt.Errorf("all roots must be absolute paths inside the allowed share root")
		}
	...
	if strings.TrimSpace(req.OutputRoot) == "" {
		req.OutputRoot = req.Roots[0]
	}
	req.OutputRoot = filepath.Clean(strings.TrimSpace(req.OutputRoot))
	if req.OutputRoot == "." || !filepath.IsAbs(req.OutputRoot) || !withinPath(s.browseBase(), req.OutputRoot) {
		return fmt.Errorf("output_root must be inside the allowed share root")
	}
	return nil
}
...
func (s *Server) reconcileAsyncStart(w http.ResponseWriter, r *http.Request) {
	...
	state.Progress = organizer.ReconcileProgress{Phase: "starting"}
	state.Summary = organizer.ReconcileSummary{}
	state.Result = organizer.ReconcileResult{}
	state.Err = ""
	state.ItemsTotal = 0
	state.ChoicesTotal = 0
	state.SnapshotDir = ""
	state.IncludeQuarantine = req.IncludeQuarantine
	state.mu.Unlock()
	...
	go func() {
		...
		if err != nil {
			...
			state.Running = false
			state.UpdatedAt = time.Now()
			state.Err = "snapshot failed: " + err.Error()
			state.Progress.Phase = "failed"
			state.mu.Unlock()
			releaseProcessMemory()
			return
		...
		state.Running = false
		state.UpdatedAt = time.Now()
		state.SnapshotDir = dir
		state.Summary = meta.Report.Summary
		state.ItemsTotal = meta.ItemsTotal
		state.ChoicesTotal = meta.ChoicesTotal
		msg := "解析完了・結果をディスクへ退避済み"
		if req.IncludeQuarantine {
			msg = "隔離フォルダを含む解析完了・結果をディスクへ退避済み"
		}
...
func (s *Server) reconcileAsyncStatus(w http.ResponseWriter, r *http.Request) {
	...
	if p.Completed > 0 && elapsed > 0 {
		rate = float64(p.Completed) / (float64(elapsed) / 1000)
		if p.Total > p.Completed && rate > 0 {
			eta = int64(float64(p.Total-p.Completed) / rate * 1000)
		}
	}
	writeJSON(w, map[string]any{
		...
		"elapsed_ms": elapsed, "estimated_remaining_ms": eta, "items_per_second": rate,
...
func (s *Server) reconcileAsyncItems(w http.ResponseWriter, r *http.Request) {
	...
	state := largeReconcile.state(mode)
	state.mu.RLock()
	dir, total := state.SnapshotDir, state.ItemsTotal
	state.mu.RUnlock()
	start, end := pageBounds(r, total)
	if dir == "" {
		writeJSON(w, map[string]any{"offset": start, "limit": 0, "total": total, "items": []organizer.ReconcileItem{}})
		return
	}
	items, err := loadSnapshotPage[organizer.ReconcileItem](dir, "items", start, end-start, total)
	if err != nil {
		http.Error(w, "result page unavailable: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *Server) reconcileAsyncChoices(w http.ResponseWriter, r *http.Request) {
	...
	state := largeReconcile.state(mode)
	state.mu.RLock()
	dir, total := state.SnapshotDir, state.ChoicesTotal
	state.mu.RUnlock()
	start, end := pageBounds(r, total)
	if dir == "" {
		writeJSON(w, map[string]any{"offset": start, "limit": 0, "total": total, "choices": []organizer.ReconcileChoice{}})
		return
	}
	items, err := loadSnapshotPage[organizer.ReconcileChoice](dir, "choices", start, end-start, total)
	if err != nil {
		http.Error(w, "choice page unavailable: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (s *Server) reconcileAsyncExecute(w http.ResponseWriter, r *http.Request) {
	if summary := s.Jobs.Summary(); summary.Running > 0 || summary.Queued > 0 {
		http.Error(w, "archive jobs are running or queued", http.StatusConflict)
		return
	}
	...
	if state.Running {
		state.mu.Unlock()
		http.Error(w, "operation already running", http.StatusConflict)
		return
	}
	if state.SnapshotDir == "" {
		state.mu.Unlock()
		http.Error(w, "run analysis first", http.StatusConflict)
		return
	}
	...
	state.StartedAt = time.Now()
	state.UpdatedAt = state.StartedAt
	state.Progress = organizer.ReconcileProgress{Phase: "starting", Message: "解析結果を読み込み中"}
	state.Result = organizer.ReconcileResult{}
	state.Err = ""
	id := fmt.Sprintf("%s-exec-%d", req.Mode, time.Now().UnixNano())
	state.ID = id
	state.mu.Unlock()
	...
	go func() {
		...
		if err == nil {
			...
			result, err = s.Organizer.ReconcileExecuteReportProgress(report, req.Selections, func(p organizer.ReconcileProgress) {
				state.mu.Lock()
				state.Progress = p
				state.UpdatedAt = time.Now()
				state.mu.Unlock()
		...
		if err != nil {
			...
			state.Progress.Message = "整理完了・RAM解放済み"
		}
		state.mu.Unlock()
		releaseProcessMemory()
	}()
	...
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.WriteHeader(http.StatusAccepted)
	writeJSON(w, map[string]any{"id": id, "mode": req.Mode})
}
...
func (s *Server) reconcileAsyncResult(w http.ResponseWriter, r *http.Request) {
	...
	state.mu.RLock()
	defer state.mu.RUnlock()
	writeJSON(w, map[string]any{"running": state.Running, "error": state.Err, "result": state.Result})
}

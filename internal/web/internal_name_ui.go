...
(function(){
  ...
  function evidenceHTML(p){
    ...
    if(cl.canonical){
      ...
      src+='<div class="muted" style="margin-top:2px">統合: '+esc(cl.canonical)+(cl.reason?' / '+esc(cl.reason):'')+(cl.score?(' '+Math.round(cl.score*100)+'%'):'')+'</div>';
      if(aliases.length>1)src+='<details class="muted"><summary>別表記 '+aliases.length+'件</summary><div style="padding-left:8px">'+aliases.map(function(x){return '<div>• '+esc(x)+'</div>';}).join('')+'</div></details>';
    }
    var cs=Array.isArray(p.candidates)?p.candidates:[],total=Number(p.candidate_count||cs.length||0);
    if(cs.length){var rows=cs.map(function(x){return '<div>• '+esc(x)+'</div>';}).join('');var more=total>cs.length?('（代表'+cs.length+'件 / 全'+total+'件）'):(''+cs.length+'件');src+='<details class="muted" style="margin-top:4px"><summary>参照した内部名 '+more+'</summary><div style="padding:5px 0 0 8px;max-width:420px;word-break:break-word">'+rows+'</div></details>';}
    return src;
  }
  ...
  window.scan=async function(){
    ...
    var body=document.getElementById('plans'),table=body?body.closest('table'):null;if(table&&table.tHead&&table.tHead.rows.length&&table.tHead.rows[0].cells.length>3)table.tHead.rows[0].cells[3].textContent='収録';
  };
})();
</script>
`

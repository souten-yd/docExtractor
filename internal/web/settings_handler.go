...
type settingsRequest struct { Root string `json:"root"` }
...
func (s *Server) getSettings(w http.ResponseWriter,r *http.Request){
	aliases:=map[string]string{};if s.Settings!=nil{aliases=s.Settings.Get().SeriesAliases};writeJSON(w,appsettings.Settings{Root:s.Organizer.Root(),SeriesAliases:aliases})
}
func (s *Server) updateSettings(w http.ResponseWriter,r *http.Request){
	if s.Settings==nil{http.Error(w,"settings store unavailable",http.StatusServiceUnavailable);return}
	r.Body=http.MaxBytesReader(w,r.Body,8*1024);var req settingsRequest;dec:=json.NewDecoder(r.Body);dec.DisallowUnknownFields();if err:=dec.Decode(&req);err!=nil{http.Error(w,"invalid request",http.StatusBadRequest);return}
	...
	oldRoot:=s.Organizer.Root();if err:=s.Organizer.SetRoot(root);err!=nil{http.Error(w,err.Error(),http.StatusBadRequest);return}
	current:=s.Settings.Get();current.Root=s.Organizer.Root();if err:=s.Settings.Save(current);err!=nil{_ = s.Organizer.SetRoot(oldRoot);http.Error(w,"failed to persist settings",http.StatusInternalServerError);return}
	if s.Diagnostics!=nil{if logger,err:=s.Diagnostics.Job("system");err==nil{_ = logger.Write(diagnostics.Event{Component:"settings",Stage:"save",Message:"archive root changed",Fields:map[string]any{"old_path":oldRoot,"new_path":s.Organizer.Root()}})}}
	writeJSON(w,current)
}
